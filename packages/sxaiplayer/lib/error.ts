...
class Errors {
   ...
   * @returns
   */
  constructor (player: any, errorInfo: ErrorInfo = { errorType: '', errorCode: 0, errorMessage: '', originError: null as any, ext: {}, mediaError: null }) {
    ...
    if (player.media) {
      ...
      if (ERROR_MAP[_errc as keyof typeof ERROR_MAP]) {
        _errc = ERROR_MAP[_errc as keyof typeof ERROR_MAP]
      }
      const r: { [propName: string]: any; } = {
