...
const pluginsManager = {
  pluginGroup: {} as Record<string, any>,
  init (player: any) {
  ...
  formatPluginInfo (plugin: any, config: any) {
    let PLUFGIN = null
    let options = null
   ...
   * @returns boolean
   */
  checkPluginIfExits (pluginName: string, plugins: Array<any>) {
   ...
   */
  getRootByConfig (pluginName: string, playerConfig: Record<string, any>) {
   ...
   *
   */
  lazyRegister (player: any, lazyPlugin: any) {
    const timeout = lazyPlugin.timeout || 1500
    return Promise.race([
      lazyPlugin.loader().then((plugin: any) => {
        let result
  ...
  **/
  register (player: any, plugin: any, options: any = {}) {
   ...
   * @param { string } name
   */
  unRegister (cgid: any, name: string) {
   ...
   * @param { string } name
   */
  deletePlugin (player: any, name: string) {
    const cgid = player._pluginInfoId
  ...
  beforeInit (player: any) {
    function retPromise (fun: any) {
      if (!fun || !fun.then) {
  ...
  setLang (lang: string, player: any) {
    const cgid = player._pluginInfoId
    if (!cgid || !this.pluginGroup[cgid]) {
      return
    }
