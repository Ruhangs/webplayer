...
function mergeIconClass(icon: any, classname: string) {
...
function mergeIconAttr(icon: any, attr: any) {
...
function createIcon(icon: any, key: string, classname: string = '', attr: Record<string, any> = {}, pluginName: string = '') {
  let newIcon: any = null
...
function registerIconsObj(iconsConfig: any, plugin: any) {
...
function registerTextObj(textConfig: any, plugin: any) {
...
class Plugin extends BasePlugin {
  /**
    ...
    * @returns { HTMLElement }
    */
  static insert(html: string | HTMLElement, parent: HTMLElement, index: number = 0): Element {
   ...
   * @returns
   */
  static delegate(root: HTMLElement, querySelector: string, eventType: string | Array<string>, callback: Function, capture: boolean = false) {
    const dels = []
  ...
  constructor(args: IUIPluginOptions = {}) {
    super(args)
    /**
     * @private
     */
  ...
  __init(args: IUIPluginOptions) {
    ...
    if (renderStr) {
      ...
      _el.setAttribute('data-index', args.index.toString())
    } else if (args.tag) {
    ...
    if (children && typeof children === 'object') {
      if (Object.keys(children).length > 0) {
        Object.keys(children).map(item => {
          ...
          const options: IUIPluginOptions = {
            root: this.root
          }
  ...
  updateLang(lang: any) {
    ...
    function checkChildren(node: any, callback: any) {
  ...
  changeLangTextKey(dom: HTMLElement, key: string = '') {
  ...
  registerPlugin(plugin: any, options: IUIPluginOptions = {}, name: string = '') {
   ...
   * @param { Function } [callback]
   */
  bind(querySelector: string | Array<string>, eventType: string | Array<string>, callback: Function) {
   ...
   * @param { string | Array<string> | Function } eventType
   */
  unbind(querySelector: string | Array<string>, eventType: string | Array<string> | Function) {
    if (arguments.length < 3 && typeof eventType === 'function') {
      if (Array.isArray(querySelector)) {
        querySelector.forEach((item) => {
  ...
  setStyle(name: any, value?: any) {
    ...
    if (Util.typeOf(name) === 'String') {
      return (this.root.style[name as string] = value)
    } else if (Util.typeOf(name) === 'Object') {
      Object.keys(name).map((key: string) => {
   ...
   * @returns
   */
  setHtml(htmlStr: string, callback: Function) {
    if (!this.root) {
      return
    }
   ...
   * @returns
   */
  bindEL(event: string, eventHandle: Function, isBubble: boolean = false) {
    if (!this.root) {
      return
    }
   ...
   * @returns
   */
  unbindEL(event: string, eventHandle: Function, isBubble: boolean = false) {
    if (!this.root) {
      return
    }
    if (`on${event}` in this.root && typeof eventHandle === 'function') {
   ...
   * @returns { HTMLElement | null }
   */
  appendChild(pdom: string | HTMLElement, child: HTMLElement) {
    if (!this.root) {
      return null
    }
